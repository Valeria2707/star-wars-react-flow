...
vi.spyOn(globalThis, "fetch").mockImplementation((url) => {
  if (url === "https://swapi.dev/api/films/1/") {
    return Promise.resolve({
      json: () => Promise.resolve({ title: "A New Hope", url }),
    } as Response);
  }
...
describe("useFilmData", () => {
  it("fetches film data for the selected person", async () => {
    const { result } = renderHook(() => useFilmData(mockPerson), {
      wrapper: ({ children }) => (
  ...
  it("returns an empty array if no person is selected", () => {
    const { result } = renderHook(() => useFilmData(null), {
      wrapper: ({ children }) => (
        <QueryClientProvider client={queryClient}>
          {children}
        </QueryClientProvider>
